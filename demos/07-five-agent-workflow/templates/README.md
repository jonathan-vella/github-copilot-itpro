# Bicep Templates - Contoso Patient Portal

The generated Bicep templates for this demo are located in:

📂 **`../../infra/bicep/contoso-patient-portal/`**

## Quick Links

- [Main Template](../../infra/bicep/contoso-patient-portal/main.bicep)
- [Deployment Script](../../infra/bicep/contoso-patient-portal/deploy.ps1)
- [Implementation Summary](../../infra/bicep/contoso-patient-portal/IMPLEMENTATION-SUMMARY.md)
- [Validation Checklist](../../infra/bicep/contoso-patient-portal/VALIDATION-CHECKLIST.md)
- [README](../../infra/bicep/contoso-patient-portal/README.md)

## File Structure

```bicep
infra/bicep/contoso-patient-portal/
├── main.bicep                      # Orchestration template
├── main.bicepparam                 # Production parameters
├── deploy.ps1                      # Deployment automation
├── README.md                       # Complete documentation
├── IMPLEMENTATION-SUMMARY.md       # Implementation details
├── VALIDATION-CHECKLIST.md         # Testing procedures
└── modules/
    ├── networking.bicep            # VNet, subnets, NSGs
    ├── monitoring.bicep            # Log Analytics, App Insights
    ├── app-service-plan.bicep      # Compute capacity
    ├── app-service.bicep           # Web application
    ├── sql-server.bicep            # SQL Server logical server
    ├── sql-database.bicep          # SQL Database with TDE
    ├── key-vault.bicep             # Secrets management
    ├── private-endpoints.bicep     # Private connectivity
    ├── key-vault-secrets.bicep     # Connection strings
    └── rbac-assignments.bicep      # Access control
```

## Quick Deploy

```powershell
cd ../../infra/bicep/contoso-patient-portal
.\deploy.ps1 -Environment prod -Location eastus2
```

## Validation

```powershell
# Build templates
bicep build main.bicep --stdout --no-restore

# Lint templates
bicep lint main.bicep

# What-if analysis
.\deploy.ps1 -WhatIf
```

---

**Note**: These templates were generated by the **bicep-implement** agent in Stage 3 of the 4-agent workflow demo.
