...
# Worker to get the SQL "client_net_address" (source IP) using sqlcmd with MSI via DSN
...
""".strip()
    ...
    query = "SET NOCOUNT ON; SELECT CAST(CONNECTIONPROPERTY('client_net_address') AS VARCHAR(50));"
    ...
    ip_line = next((line for line in stdout.splitlines() if line.strip()), "")
    if not ip_line:
        print(json.dumps({"error": "No output from sqlcmd", "stdout": stdout[:300], "ok": False}))
        return

    print(json.dumps({"source_ip": ip_line, "ok": True, "mode": "sqlcmd-msi-dsn"}))
